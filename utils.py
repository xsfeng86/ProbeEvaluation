...
def slice_recording(recording,e):
    ...
    j = 1
    ch_start.append(recording.get_num_channels())
...
def generate_probe(ChannelmapPath,CMname,RecordingSystem,FPC,WO_record_channel_ids):
    ...
    #得到的channel map是序号连续的，需要另外有通道编号，才能对应到记录系统的编号里，对于128正常，对于64通道需要另外生成
    ...
    if column_name:
        ...
        all_ch_map  = ChannelMapDf.sort_values(by=column_name[0])[RF].to_numpy()
        # channel map,若为少于64通道去除无效通道
        ch_map =all_ch_map[:probe_dict['Channel_number']]
        # 无效通道的编号 0-index,在把信号加上probe前一定要筛选掉通道！！
        invld_ch_ids =all_ch_map[probe_dict['Channel_number']:]
        ...
        #和intan对应的绝对通道编号，不一定连续
        vld_ch_map_abs = prb_df_vld['device_channel_indices'].to_numpy()
         #绝对通道编号的index（相对通道编号，必须连续否则无法sorting
        vld_ch_map = vld_ch_map_abs.argsort().argsort()
        probe2 = probe1.from_dataframe(prb_df_vld)
        probe2.set_device_channel_indices(vld_ch_map)
    else:
        print(f"没有{CMname}对应的Channelmap！")

    return probe2,probe_dict,invld_ch_ids
...
## 对于单个文件夹中的rhd文件：
def single_electrode_analysis(RecordingPath,j):
...
#     ## 飞书新增记录，返回该记录token
    ...
    probe,probe_dict,invld_ch_ids = generate_probe(ChannelmapPath,Probe_version,RecordingSystem,FPC_version,WO_record_channel_ids)
    ...
    ##对于少于128通道的电极，把连空的通道去掉才可以使用
    unconnected_channel_ids = list((set(invld_ch_ids))&set(recording_part.channel_ids))
    recording_part = recording_part.remove_channels(remove_channel_ids=unconnected_channel_ids)
    ...
    del sorting, recording_cmr,analyzer
